...
public class ChatAssistantService {
    ...
    private final RagEnhancedChatService ragEnhancedChatService;
    ...
    public ChatResponse chat(List<ChatMessage> messages, String scene) {
        try {
            ...
            request.setEnableRag(true); // 默认启用RAG
            
            // 执行RAG增强对话
            RagChatResponse ragResponse = ragEnhancedChatService.ragChat(request);
            
            // 转换为旧接口响应格式
            if (ragResponse.getSuccess()) {
                return ChatResponse.success(ragResponse.getContent(), ragResponse.getTokensUsed());
            } else {
                return ChatResponse.fail(ragResponse.getError());
            }
        } catch (Exception e) {
            log.error("RAG增强对话失败，回退到传统模式: {}", e.getMessage());
            // 回退到传统模式
            return fallbackChat(messages, scene);
        }
    }
     ...
     * 传统聊天模式（回退方案）
     */
    private ChatResponse fallbackChat(List<ChatMessage> messages, String scene) {
        // 根据场景选择系统提示词
        String systemPrompt = getSystemPromptByScene(scene);
     ...
     * 根据场景获取系统提示词
     */
    private String getSystemPromptByScene(String scene) {
        // 尝试从Prompt模板系统获取
        String templateScene = promptTemplateService.getSceneMapping().get(scene);
        if (templateScene != null) {
            String renderedPrompt = promptTemplateService.renderTemplate(templateScene, null);
    ...
    public String quickAnswer(String question) {
        return quickAnswer(question, null);
    }
    ...
    public String quickAnswer(String question, Long userId) {
        ...
        try {
            return ragEnhancedChatService.quickAnswer(question, userId);
        } catch (Exception e) {
            log.error("快速问答失败: {}", e.getMessage());
    ...
    public String generateLessonPlan(String subject, String studentLevel, String lessonDuration, String studentInfo) {
        return generateLessonPlan(subject, studentLevel, lessonDuration, studentInfo, null);
    }
     ...
     * @return 教案内容
     */
    public String generateLessonPlan(String subject, String studentLevel, String lessonDuration, 
                                    String studentInfo, Long userId) {
        if (!StringUtils.hasText(subject) || !StringUtils.hasText(studentLevel)) {
            throw new IllegalArgumentException("科目和学生水平不能为空");
        }
        
        try {
            return ragEnhancedChatService.generateLessonPlan(subject, studentLevel, lessonDuration, studentInfo, userId);
        } catch (Exception e) {
            log.error("生成教案失败: {}", e.getMessage(), e);
            throw new RuntimeException("AI教案生成失败：" + e.getMessage());
        }
    }
    ...
    public String polishComment(String rawComment, String subject, String studentInfo) {
        return polishComment(rawComment, subject, studentInfo, null);
    }
     ...
     * @return 润色后的评语
     */
    public String polishComment(String rawComment, String subject, String studentInfo, Long userId) {
        if (!StringUtils.hasText(rawComment)) {
            throw new IllegalArgumentException("原始评语不能为空");
        }
        
        try {
            return ragEnhancedChatService.polishComment(rawComment, subject, studentInfo, userId);
        } catch (Exception e) {
            log.error("润色评语失败: {}", e.getMessage(), e);
            throw new RuntimeException("AI评语润色失败：" + e.getMessage());
        }
    }
    ...
    public RagChatResponse ragChat(RagChatRequest request) {
        return ragEnhancedChatService.ragChat(request);
    }
}
