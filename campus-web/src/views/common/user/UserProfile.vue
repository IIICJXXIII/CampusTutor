  ...
  <div class="user-profile-page">
    ...
    <div class="profile-card" v-if="profile" v-loading="loading">
      <div class="profile-header">
        <el-avatar :size="72" :src="profile.avatarUrl || undefined" class="profile-avatar">
          {{ profile.nickname?.charAt(0) || '?' }}
        </el-avatar>
        <h2 class="profile-name">{{ profile.nickname || '未知用户' }}</h2>
        <el-tag size="small" :type="profile.role === 1 ? 'success' : profile.role === 2 ? 'warning' : 'info'">
          {{ roleMap[profile.role] || '用户' }}
        </el-tag>
      </div>
      ...
      <div class="profile-info">
        <div class="info-item" v-if="profile.gender">
          ...
          <span class="info-value">{{ profile.gender === 1 ? '男' : '女' }}</span>
        </div>
        <div class="info-item" v-if="profile.region">
          <span class="info-label">地区</span>
          <span class="info-value">{{ profile.region }}</span>
        </div>
        <div class="info-item" v-if="profile.address">
          <span class="info-label">地址</span>
          <span class="info-value">{{ profile.address }}</span>
        </div>
        <div class="info-item" v-if="profile.createTime">
